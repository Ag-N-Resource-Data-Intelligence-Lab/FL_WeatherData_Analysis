...
DtF_FAWN=read.csv('C:/Users/Chi Zhang/Desktop/PhD_Second/Precipitation Code/Florida_hourly_FAWN.csv',
...
# rain events
Climate_Dt %>% 
  Precip_Evt_Sep(.,interval,IntE_P) -> DtF_FAWN_sep
...
DtF_FAWN_sep %>% 
  ...
  filter(TotalRain>0) -> DtF_FAWN_sep_rain

write.table(DtF_FAWN_sep_rain, './Rain_Evt.csv',row.names = FALSE,sep = ',')
...
#=======
mutate(PreRain_Dur_hr=lag(Dur_hr)) %>% 
  filter(TotalRain==0) -> DtF_FAWN_sep_dry
