...
breaks='1 days'
Raw_dt_all_loc_fawn %>% 
  filter(data.table::between(Time,ymd_hms('2015-06-13 15:00:00'),ymd_hms('2015-06-18 17:00:00')), Location == 'MACCLENNY') %>% 
  ...
  geom_line(aes(y=RH,color='RH'),size=1)+ 
...
breaks='1 days'
Raw_dt_all_loc_fawn %>% 
  filter(data.table::between(Time,ymd_hms('2014-06-02 21:00:00'),ymd_hms('2014-06-06 04:00:00')), Location == 'CITRA') %>% 
  mutate(Pressure_chng.av=as.numeric(Pressure_chng.av),
         Rain=as.numeric(Rain),
         Temp_C=as.numeric(Temp_C),
         RH=as.numeric(RH),
...
breaks='1 days'
Raw_dt_all_loc_fawn %>% 
  filter(data.table::between(Time,ymd_hms('2014-11-24 14:00:00'),ymd_hms('2014-11-29 01:00:00')), Location == 'MACCLENNY') %>% 
  mutate(Pressure_chng.av=as.numeric(Pressure_chng.av),
         Rain=as.numeric(Rain),
         Temp_C=as.numeric(Temp_C),
         RH=as.numeric(RH),
...
breaks='1 days'
Raw_dt_all_loc_fawn %>% 
  filter(data.table::between(Time,ymd_hms('2011-12-03 18:00:00'),ymd_hms('2011-12-07 20:00:00')), Location == 'PIERSON') %>% 
  mutate(Pressure_chng.av=as.numeric(Pressure_chng.av),
         Rain=as.numeric(Rain),
         Temp_C=as.numeric(Temp_C),
         RH=as.numeric(RH),
...
ggsave(file="./plot/fawn/normal/PCE vs Rain Prob and Depth_ncdc.jpg", g, width=10,height=10)
...
ggsave(file="./plot/fawn/normal/Monthly temperature vs PCE frequency_ncdc.jpg", width=10,height=7)
...
ggsave(file="./plot/fawn/normal/AMT vs PCE Rain Prob facet on PCE types_ncdc.jpg", width=10,height=6)
