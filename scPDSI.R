...
Get_Press_Evt=function(Dt)
{
  ...
  Dt %>% 
    ...
    mutate(Yr=year(St),
           Mon=month(St)) %>% 
    inner_join(.,Dt_MonT,by=c('Yr'='Yr','Mon'='Mon')) %>% 
    ungroup %>% 
    arrange(St) %>% 
...
data_preprosessing = function(DtF)
{
  ...
  DtF %>% 
    Regular_Time(.,interval) -> DtF_clean
  # 
  # DtF_clean %>% 
...
data_preprosessing(DtF_NCDC) -> Dt_new
Dt_new %>% filter(!is.na(Dt_new$Location)) -> DtF_clean_loc
...
unique(Dtf_pdsi) -> DtF_clean_pdsi 
DtF_clean_pdsi %>% rename(rain= 'sum(Rain)') -> DtF_clean_pdsi
DtF_clean_pdsi %>% 
  filter(Location == 'JACKSONVILLE INTERNATIONAL AIRPORT, FL US') -> Jax

#plot scpdsi
P <- Jax$rain
PE <- thornthwaite(Jax$MonT, 27.994402)
sc_pdsi <- pdsi(P, PE, start = 1948)

plot(sc_pdsi)
axis(side = 1, at = seq(1930,2020, by=10))

#all locations


P <- DtF_clean_pdsi$
PE <- thornthwaite(DtF_clean_pdsi$MonT, 27.994402)
sc_pdsi <- pdsi(P, PE, start = 1948)

plot(sc_pdsi, axes=FALSE)
axis(side = 1, at = seq(1930,2020, by=10))
